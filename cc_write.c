...
void helpo()
{
  fprintf(stderr, "usage : cc_write -g gpio_device [-d pin_DD] [-c pin_DC] [-r pin_reset] file_to_flash\n");
...
int main(int argc, char *argv[])
{
  ...
  while (fgets(buffer, 600, ficin))
  {
    ...
    for (i = 0; i < len; i++)
    {
      if (!sscanf(buffer + 9 + 2 * i, "%02hhx", &data[i]))
      {
    ...
    {                       // copy end of datas to next page
      ...
      if (0 < Pages[page + 1].minoffset)
        Pages[page + 1].minoffset = 0;
      if ((start + len - 2048 - 1) > Pages[page].maxoffset)
        Pages[page].maxoffset = start + len - 2048 - 1;
      len = 2048 - start;
    }
    memcpy(&Pages[page].datas[start], data, len);
    if (start < Pages[page].minoffset)
      Pages[page].minoffset = start;
    if ((start + len - 1) > Pages[page].maxoffset)
      Pages[page].maxoffset = start + len - 1;
  }
  ...
  {
    provo++;
    if (Pages[page].maxoffset < Pages[page].minoffset)
      continue;
